...
export class AbsensiChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      let pie1 = [];
      let pie2Memenuhi = [];
      let pie2TidakMemenuhi = [];
      let pie3 = [];
      const domain3 = [];
      const domain1 = [];
      const domain2 = [];
      let domainMemenuhi = [];
      let domainTidakMemenuhi = [];
      ...
      domain1.push(["check_in", ">=", startDate]);
      domain1.push(["check_in", "<=", endDate]);
      domain2.push(["check_in", ">=", startDate]);
      domain2.push(["check_in", "<=", endDate]);

      domainMemenuhi.push(["overtime_hours", ">=", 0]);
      domainTidakMemenuhi.push(["overtime_hours", "<", 0]);
      domainMemenuhi.push(["check_in", ">=", startDate]);
      domainMemenuhi.push(["check_in", "<=", endDate]);
      domainTidakMemenuhi.push(["check_in", ">=", startDate]);
      domainTidakMemenuhi.push(["check_in", "<=", endDate]);

      pie1 = await this.orm.call("hr.attendance", "search_read", [
        domain1,
        ["id", "overtime_status"],
      ]);

      pie2Memenuhi = await this.orm.call("hr.attendance", "search_read", [
        domainMemenuhi,
        ["id", "employee_id"],
      ]);

      pie2TidakMemenuhi = await this.orm.call("hr.attendance", "search_read", [
        domainTidakMemenuhi,
        ["id", "employee_id"],
      ]);
      ...
      await this.processData(pie1, pie2Memenuhi, pie2TidakMemenuhi, pie3);
    } catch (error) {
  ...
  async processData(pie1, pie2Memenuhi, pie2TidakMemenuhi, pie3) {
    const aggregateDataByLembur = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.overtime_status;
    ...
    const processDataPie2 = (memenuhi, tidakMemenuhi) => {
      const stateCounts = {
        "Memenuhi Jam Kerja": {
          count: memenuhi.length,
          ids: memenuhi.map((record) => record.id),
        },
        "Tidak Memenuhi Jam Kerja": {
          count: tidakMemenuhi.length,
          ids: tidakMemenuhi.map((record) => record.id),
        },
      };
      return stateCounts;
    };
    ...
    if (this.props.title === "pie1") {
      const stateCounts = aggregateDataByLembur(pie1);
      this.state.originalLabels = Object.keys(stateCounts).map((state) => {
        if (state === "to_approve") return "Akan Disetujui";
      ...
      const stateCounts = processDataPie2(pie2Memenuhi, pie2TidakMemenuhi);

      this.state.originalLabels = Object.keys(stateCounts);
      this.state.labels = this.state.originalLabels;
  ...
  // Helper method to check data availability
  ...
  // Helper method to handle empty state
  handleEmptyState() {
    this.hasData = false;
    console.log("hasdata: ", this.hasData);
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      const originalLabel = this.state.originalLabels[firstPoint.index];
      ...
      let resModel = "hr.attendance";
      let domainAction = [];
      console.log("Label Yang di Klik", label);

      if (this.props.title === "pie1") {
        let statusLembur = originalLabel;
        let convertLabel;

        if (statusLembur === "Akan Disetujui") {
          convertLabel = "to_approve";
        } else if (statusLembur === "Disetujui") {
          convertLabel = "approved";
        } else if (statusLembur === "Ditolak") {
          convertLabel = "refused";
        }

        if (convertLabel) {
          domainAction.push(
            ["id", "in", associatedIds],
            ["overtime_status", "=", convertLabel]
          );
        }
      } else if (this.props.title === "pie2") {
        let statusJamKerja = originalLabel;
        if (statusJamKerja === "Tidak Memenuhi Jam Kerja") {
          domainAction.push(["overtime_hours", "<=", 0]);
          domainAction.push(["id", "in", associatedIds]);
          domainAction.push(["check_in", ">=", this.state.startDate2]);
          domainAction.push(["check_in", "<=", this.state.endDate2]);
        } else if (statusJamKerja === "Memenuhi Jam Kerja") {
          domainAction.push(["overtime_hours", ">=", 0]);
          domainAction.push(["id", "in", associatedIds]);
          domainAction.push(["check_in", ">=", this.state.startDate2]);
          domainAction.push(["check_in", "<=", this.state.endDate2]);
...
AbsensiChartRenderer.template = "owl.AbsensiChartRenderer";
