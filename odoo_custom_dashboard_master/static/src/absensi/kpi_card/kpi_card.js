...
export class AbsensiKpiCard extends Component {
  setup() {
  ...
  async updateKpiData() {
    ...
    try {
      ...
      if (this.state.startDate) {
        domain.push(["check_in", ">=", this.state.startDate]);
        domain1.push(["check_in", ">=", this.state.startDate]);
        domain2.push(["check_in", ">=", this.state.startDate]);
        domain3.push(["check_in", ">=", this.state.startDate]);
      }
      if (this.state.endDate) {
        domain.push(["check_in", "<=", this.state.endDate]);
        domain1.push(["check_in", "<=", this.state.endDate]);
        domain2.push(["check_in", "<=", this.state.endDate]);
        domain3.push(["check_in", "<=", this.state.endDate]);
      }

      // ,

      domain1.push(["overtime_hours", ">=", 0]);
      domain2.push(["overtime_hours", "<", 0]);
      // domain2.push(["overtime_hours", "<=", 0]);
      domain3.push(["overtime_status", "=", "refused"]);
      domain.push(["overtime_status", "=", "approved"]);
      // domain1.push(["invoice_status", "!=", "no"]);

      let memenuhiData = await this.orm.call("hr.attendance", "search_read", [
        domain1,
        ["id", "employee_id"],
      ]);

      let tidakMemenuhiData = await this.orm.call(
        "hr.attendance",
        "search_read",
        [domain2, ["id", "overtime_hours"]]
      );

      let lemburTidakDisetujui = await this.orm.call(
        "hr.attendance",
        "search_read",
        [domain3, ["id", "overtime_status"]]
      );

      let lemburDisetujui = await this.orm.call(
        "hr.attendance",
        "search_read",
        [domain, ["id", "overtime_status"]]
      );
      ...
      // let penawaranTerbaru = 1;

      // let orderTotal = orderData.length;
      let terpenuhi = memenuhiData.length;
      let tidakTerpenuhi = tidakMemenuhiData.length;
      let tidakDisetujui = lemburTidakDisetujui.length;
      let disetujui = lemburDisetujui.length;

      // console.log("Penawaran Terbaru", penawaran);

      this.state.kpiData = [
        {
          name: "Memenuhi Jam Kerja",
          value: terpenuhi,
        ...
        {
          name: "Tidak Memenuhi Jam Kerja",
          value: tidakTerpenuhi,
          icon: "fa-user-clock",
          res_model: "hr.attendance",
          domain: domain2,
        ...
        {
          //<i class="fa-solid "></i>
          name: "Lembur Tidak disetujui",
          value: tidakDisetujui,
...
AbsensiKpiCard.template = "owl.AbsensiKpiCard";
