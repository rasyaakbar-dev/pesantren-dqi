...
export class CrmKpiCard extends Component {
  setup() {
  ...
  //   try {
  //     const domainLead = [];
  //     const domainCustomer = [];
  //     const domainSales = [];
  //     const domainRevenue = [];

  //     // Menambahkan filter startDate & endDate
  //     if (this.state.startDate) {
  //       const startDateFilter = ["create_date", ">=", this.state.startDate];
  //       domainLead.push(startDateFilter);
  //       domainCustomer.push(startDateFilter);
  //       domainSales.push(startDateFilter);
  //       domainRevenue.push(startDateFilter);
  //     }
  //     if (this.state.endDate) {
  //       const endDateFilter = ["create_date", "<=", this.state.endDate];
  //       domainLead.push(endDateFilter);
  //       domainCustomer.push(endDateFilter);
  //       domainSales.push(endDateFilter);
  //       domainRevenue.push(endDateFilter);
  //     }

  //     // Query data dengan Promise.all untuk efisiensi
  //     const [leadsData, customersData, salesData, revenueData] =
  //       await Promise.all([
  //         this.orm.call("crm.lead", "search_read", [domainLead, ["id"]]),
  //         this.orm.call("res.partner", "search_read", [domainCustomer, ["id"]]),
  //         this.orm.call("sale.order", "search_read", [domainSales, ["id"]]),
  //         this.orm.call("sale.order", "search_read", [
  //           domainRevenue,
  //           ["amount_total"],
  //         ]),
  //       ]);
  ...
  //     let totalRevenue = revenueData.reduce(
  ...
  //         value: this.formatLargeNumber(totalRevenue),
  //         icon: "fa-dollar-sign",
  //         res_model: "sale.order",
  //         domain: domainRevenue,
  //       },
  //     ];

  //     // Apply animasi ke setiap KPI Card
  ...
  async updateKpiData() {
    ...
    try {
      const domainLeads = [];
      const domainOpenOpportunities = [
        ["stage_id", "!=", "lost"],
        ["stage_id", "!=", "won"],
      ];
      const domainClosedDeals = [["stage_id", "=", "Berhasil"]];
      const domainRevenueForecast = [["state", "not in", ["sale", "cancel"]]];

      if (this.state.startDate) {
        const startDateFilter = ["create_date", ">=", this.state.startDate];
        domainLeads.push(startDateFilter);
        domainOpenOpportunities.push(startDateFilter);
        domainClosedDeals.push(startDateFilter);
        domainRevenueForecast.push(startDateFilter);
      }
      if (this.state.endDate) {
        const endDateFilter = ["create_date", "<=", this.state.endDate];
        domainLeads.push(endDateFilter);
        domainOpenOpportunities.push(endDateFilter);
        domainClosedDeals.push(endDateFilter);
        domainRevenueForecast.push(endDateFilter);
      }

      const [
        leadsData,
        openOpportunitiesData,
        closedDealsData,
        revenueForecastData,
      ] = await Promise.all([
        this.orm.call("crm.lead", "search_read", [domainLeads, ["id"]]),
        this.orm.call("crm.lead", "search_read", [
          domainOpenOpportunities,
          ["id"],
        ]),
        this.orm.call("crm.lead", "search_read", [domainClosedDeals, ["id"]]),
        this.orm.call("sale.order", "search_read", [
          domainRevenueForecast,
          ["amount_total"],
        ]),
      ]);
      ...
      // Menghitung KPI
      let totalLeads = leadsData.length;
      let totalOpenOpportunities = openOpportunitiesData.length;
      let totalClosedDeals = closedDealsData.length;
      let totalRevenueForecast = revenueForecastData.reduce(
        (total, order) => total + (order.amount_total || 0),
        0
      );
      ...
      this.state.kpiData = [
        ...
        {
          name: "Peluang Terbuka",
          value: totalOpenOpportunities,
          icon: "fa-clipboard-list",
          res_model: "crm.lead",
          domain: domainOpenOpportunities,
        },
        {
          name: "Kesepakatan Berhasil",
          value: totalClosedDeals,
          icon: "fa-handshake",
          res_model: "crm.lead",
          domain: domainClosedDeals,
        },
        {
          name: "Perkiraan Pendapatan",
          value: this.formatLargeNumber(totalRevenueForecast),
          icon: "fa-chart-line",
          res_model: "sale.order",
          domain: domainRevenueForecast,
        },
      ];
...
CrmKpiCard.template = "owl.CrmKpiCard";
