...
export class CutiChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      pie2 = await this.orm.call("hr.leave", "search_read", [
        domain,
        ["id", "employee_id", "holiday_status_id"],
      ]);

      pie3 = await this.orm.call("hr.leave", "search_read", [
        domain,
        ["id", "employee_id", "duration_display"],
      ]);
  ...
  async processData(pie1, pie2, pie3) {
    ...
    const aggregateDataByHolidayStatus = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.holiday_status_id;
        if (!stateCounts[state]) {
          stateCounts[state] = { count: 0, ids: [] };
        }
        stateCounts[state].count += 1;
    ...
    const aggregateDataByDuration = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.duration_display;
        if (!stateCounts[state]) {
          stateCounts[state] = { count: 0, ids: [] };
        }
        stateCounts[state].count += 1;
    ...
    if (this.props.title === "pie1") {
      ...
      this.state.labels = Object.keys(stateCounts).map((state) => {
        if (state === "confirm") return "Akan Disetujui";
        if (state === "refuse") return "Ditolak";
        if (state === "validate1") return "Persetujuan Kedua";
        if (state === "validate") return "Disetujui";
      ...
      const stateCounts = aggregateDataByHolidayStatus(pie2);
      ...
      const stateCounts = aggregateDataByDuration(pie3);
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      if (this.props.title === "pie1") {
        ...
        if (label === "Akan Disetujui") {
          convertLabel = "confirm";
        } else if (label === "Ditolak") {
          convertLabel = "refuse";
        } else if (label === "Persetujuan Kedua") {
          convertLabel = "validate1";
        } else if (label === "Disetujui") {
          convertLabel = "validate";
        } else {
          convertLabel = "Dibatalkan";
        }
        if (convertLabel) {
          domainAction.push(
            ["id", "in", associatedIds],
        ...
        domainAction.push(
          ["id", "in", associatedIds],
          ["holiday_status_id", "=", label]
        );
      } else if (this.props.title === "pie3") {
        domainAction.push(
          ["id", "in", associatedIds],
          ["duration_display", "=", label]
        );
      }
...
CutiChartRenderer.template = "owl.CutiChartRenderer";
