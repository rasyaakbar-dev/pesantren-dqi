...
export class KaryawanList extends Component {
  setup() {
  ...
  async fetchAllProducts() {
    try {
      ...
      let domainKaryawan = [];
      domainKaryawan.push(["department_id", "!=", false]);
      domainKaryawan.push(["job_id", "!=", false]);
      // if (this.state.startDate && this.state.endDate) {
      ...
      const karyawanData = await this.orm.call("hr.employee", "search_read", [
        // dateDomain,
        domainKaryawan,
        ["id", "name", "work_email", "department_id", "job_id"],
      ]);

      console.log("Fetched Karyawan Data:");
      this.state.karyawann = this.getKaryawanData(karyawanData);
      await this.render();
    } catch (error) {
...
KaryawanList.template = "owl.KaryawanList";
