...
export class KaryawanChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      // ]);
      pie1 = await this.orm.call("hr.employee", "search_read", [
        domain1,
        ["id", "name", "department_id"],
      ]);

      pie2 = await this.orm.call("hr.employee", "search_read", [
        domain2,
        ["id", "name", "job_id"],
      ]);
  ...
  async processData(pie1, pie2, pie3) {
    ...
    const aggregateDataByEmployee = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.department_id;
        if (!stateCounts[state]) {
          stateCounts[state] = { count: 0, ids: [] };
        }
        stateCounts[state].count += 1;
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      if (this.props.title === "pie1") {
        ...
        domainAction.push(
          ["id", "in", associatedIds],
          ["department_id", "=", departementName]
        );
      } else if (this.props.title === "pie2") {
        let namaPekerjaan = originalLabel;
        domainAction.push(
          ["id", "in", associatedIds],
          ["job_id", "=", namaPekerjaan]
        );
...
KaryawanChartRenderer.template = "owl.KaryawanChartRenderer";
