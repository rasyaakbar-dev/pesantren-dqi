...
export class PembelianChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      domain1.push(["date_approve", ">=", startDate]);
      domain1.push(["date_approve", "<=", endDate]);
      domain2.push(["date_order", ">=", startDate]);
      domain2.push(["date_order", "<=", endDate]);

      pie1 = await this.orm.call("purchase.order", "search_read", [
        domain1,
        ["id", "invoice_status"],
      ]);

      pie2 = await this.orm.call("purchase.order", "search_read", [
        domain2,
  ...
  async processData(pie1, pie2, pie3) {
    const aggregateDataByTagihan = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
    ...
    if (this.props.title === "pie1") {
      const stateCounts = aggregateDataByTagihan(pie1);

      this.state.originalLabels = Object.keys(stateCounts).map((state) => {
        if (state === "no") return "Belum";
      ...
      this.state.originalLabels = Object.keys(stateCounts).map((state) => {
        if (state === "draft") return "Ppw";
        if (state === "sent") return "Ppw Dikirim";
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      let resModel = "purchase.order";
      let domainAction = [];
      console.log("Label Yang di Klik", label);

      if (this.props.title === "pie1") {
        let statusTagihan = originalLabel;
        let convertLabel;

        if (statusTagihan === "Sepenuhnya Ditagih") {
          convertLabel = "invoiced";
        } else if (statusTagihan === "Siap Difakturkan") {
          convertLabel = "to invoice";
        } else if (statusTagihan === "Belum") {
          convertLabel = "no";
        }
        ...
        let pembayaran = originalLabel;
        let convertLabel;

        if (pembayaran === "Ppw") {
          convertLabel = "draft";
        } else if (pembayaran === "Ppw Dikirim") {
          convertLabel = "sent";
        } else if (pembayaran === "Akan Disetujui") {
          convertLabel = "to approve";
        } else if (pembayaran === "Order Pembelian") {
...
PembelianChartRenderer.template = "owl.PembelianChartRenderer";
