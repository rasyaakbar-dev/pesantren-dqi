...
export class PenagihanChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      domain1.push(["state", "!=", "draft"]);

      domain4.push(["invoice_date", ">=", startDate]);
  ...
  async processData(pie1, pie2, pie3) {
    const aggregateDataByState = (data) => {
      ...
      data.forEach((record) => {
        ...
        // Sum up the amounts (with null check)
        stateCounts[state].amount += record.amount_company_currency_signed || 0;
    ...
    const aggregateDataByStatus = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.payment_state;
        if (!stateCounts[state]) {
          stateCounts[state] = {
            amount: 0,
            ids: [],
    ...
    if (this.props.title === "pie1") {
      const stateCounts = aggregateDataByStatus(pie1);
      ...
      this.state.datasets = [
        {
          label: "Total",
          data: Object.values(stateCounts).map((item) => item.amount),
      ...
      this.state.originalLabels = Object.keys(stateCounts).map((state) => {
        if (state === "in_process") return "Dalam Proses";
        if (state === "paid") return "Lunas";
        if (state === "partial") return "Terbayar Sebagian";
        if (state === "draft") return "Draft";
        if (state === "canceled") return "Dibatalkan";
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      if (this.props.title === "pie1") {
        let statusLunas = originalLabel;
        let convertLabel;
        if (statusLunas === "Belum Lunas") {
        ...
        let convertLabel;
        if (label === "Dalam Proses") {
          convertLabel = "in_process";
        } else if (label === "Lunas") {
          convertLabel = "paid";
...
PenagihanChartRenderer.template = "owl.PenagihanChartRenderer";
