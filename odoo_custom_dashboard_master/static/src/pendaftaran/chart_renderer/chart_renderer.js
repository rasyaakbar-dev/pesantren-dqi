...
export class PendaftaranChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      domain1.push(["tanggal_daftar", ">=", startDate]);
      domain1.push(["tanggal_daftar", "<=", endDate]);

      domain2.push(["tanggal_daftar", ">=", startDate]);
      domain2.push(["tanggal_daftar", "<=", endDate]);
      ...
      // ]);
      pie1 = await this.orm.call("ubig.pendaftaran", "search_read", [
        domain1,
        ["id", "state"],
      ]);

      pie2 = await this.orm.call("ubig.pendaftaran", "search_read", [
        domain2,
        ["id", "jenjang"],
      ]);

      // pie3 = await this.orm.call("account.move", "search_read", [
      //   domain,
  ...
  async processData(pie1, pie2) {
    const aggregateDataByJobId = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.jenjang;
    ...
    const aggregateDataByEmployee = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
    ...
    if (this.props.title === "pie1") {
      ...
      this.state.originalLabels = Object.keys(stateCounts).map((state) => {
        if (state === "diterima") return "Diterima";
        if (state === "seleksi") return "Diseleksi";
        if (state === "ditolak") return "Tidak Diterima";
        if (state === "terdaftar") return "Terdaftar";
        if (state === "draft") return "Draft";
        if (state === "batal") return "Dibatalkan";
      });
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      const resModel = "ubig.pendaftaran";
      ...
      if (this.props.title === "pie1") {
        let status = originalLabel;

        let convertLabel;
        if (status === "Draft") {
          convertLabel = "draft";
        } else if (status === "Diseleksi") {
          convertLabel = "seleksi";
        } else if (status === "Diterima") {
          convertLabel = "diterima";
        } else if (status === "Tidak Diterima") {
          convertLabel = "ditolak";
        } else if (status === "Terdaftar") {
          convertLabel = "terdaftar";
...
PendaftaranChartRenderer.template = "owl.PendaftaranChartRenderer";
