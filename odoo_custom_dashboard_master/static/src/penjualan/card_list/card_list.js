...
export class PenjualanList extends Component {
  setup() {
  ...
  async fetchAllProducts() {
    try {
      ...
      if (this.state.startDate && this.state.endDate) {
        domainPenawaran.push(["date_order", ">=", this.state.startDate]);
        domainPenawaran.push(["date_order", "<=", this.state.endDate]);
      }

      domainPenawaran.push(["state", "=", "draft"]);

      const penawaranData = await this.orm.call("sale.order", "search_read", [
        domainPenawaran,
        ["id", "name", "partner_id", "user_id", "amount_total", "state"],
      ]);

      console.log("Fetched Penawaran Data:", penawaranData);
      this.state.permintaann = this.getPermintaanTerbaru(penawaranData);
      await this.render();
    } catch (error) {
  ...
  redirectToFakturList(fakturId) {
    try {
      console.log(fakturId);
  ...
  onFakturClick(fakturId) {
    this.redirectToFakturList(fakturId);
  }
}

export class CardListPenjualan extends Component {
  setup() {
    this.orm = useService("orm");
    this.action = useService("action");
    const today = new Date();
  ...
  async fetchAllProducts() {
    try {
      let domainDifakturkan = [];

      if (this.state.startDate && this.state.endDate) {
        domainDifakturkan.push(["date_order", ">=", this.state.startDate]);
        domainDifakturkan.push(["date_order", "<=", this.state.endDate]);
      }

      domainDifakturkan.push(["invoice_status", "=", "to invoice"]);

      const fakturData = await this.orm.call("sale.order", "search_read", [
        domainDifakturkan,
        [
          "id",
          "name",
      ...
      console.log("Fetched Permintaan Data:", fakturData);
      this.state.permintaann = this.getFakturList(fakturData);
      await this.render();
    } catch (error) {
      console.error("Error fetching Karyawan Data:", error);
    }
...
PenjualanList.template = "owl.PenjualanList";
CardListPenjualan.template = "owl.CardListPenjualan";
