...
export class PenjualanChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      domain1.push(["date_order", ">=", startDate]);
      domain1.push(["date_order", "<=", endDate]);
      domain2.push(["date_order", ">=", startDate]);
      domain2.push(["date_order", "<=", endDate]);

      pie1 = await this.orm.call("sale.order", "search_read", [
        domain1,
        ["id", "state"],
      ]);

      pie2 = await this.orm.call("sale.order", "search_read", [
        domain2,
        ["id", "invoice_status"],
      ]);
      ...
      console.log("Pie1 Data", pie1);
      console.log("Pie 2 Data", pie2);
  ...
  async processData(pie1, pie2) {
    ...
    const aggregateDataByFaktur = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.invoice_status;
        if (!stateCounts[state]) {
          stateCounts[state] = { count: 0, ids: [] };
        }
        stateCounts[state].count += 1;
    ...
    if (this.props.title === "pie1") {
      ...
      this.state.labels = Object.keys(stateCounts).map((state) => {
        if (state === "draft") return "Penawaran";
        if (state === "sent") return "Penawaran Terkirim";
        if (state === "sale") return "Order Penjualan";
      ...
      const stateCounts = aggregateDataByFaktur(pie2);

      this.state.labels = Object.keys(stateCounts).map((state) => {
        if (state === "upselling") return "Peluang Penjualan Upsell";
        if (state === "to invoice") return "Siap Difakturkan";
        if (state === "invoiced") return "Sepenuhnya Ditagih";
        if (state === "no") return "Belum Selesai";
      });
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      let resModel = "sale.order";
      let domainAction = [];

      if (this.props.title === "pie1") {
        let statusPenawaran = label;
        let convertLabel;

        if (statusPenawaran === "Penawaran") {
          convertLabel = "draft";
        } else if (statusPenawaran === "Penawaran Terkirim") {
          convertLabel = "sent";
        } else if (statusPenawaran === "Order Penjualan") {
          convertLabel = "sale";
        } else {
          convertLabel = "cancel";
        }
        ...
        if (convertLabel) {
          domainAction.push(
            ["state", "=", convertLabel],
            ["date_order", ">=", this.state.startDate2],
            ["date_order", "<=", this.state.endDate2]
          );
        }
      } else if (this.props.title === "pie2") {
        let statusFaktur = label;
        let convertLabel;

        if (statusFaktur === "Peluang Penjualan Upsell") {
          convertLabel = "upselling";
        } else if (statusFaktur === "Siap Difakturkan") {
          convertLabel = "to invoice";
        } else if (statusFaktur === "Sepenuhnya Ditagih") {
          convertLabel = "invoiced";
        } else if (statusFaktur === "Belum Selesai") {
          convertLabel = "no";
        }
        if (convertLabel) {
          domainAction.push(
            ["invoice_status", "=", convertLabel],
            ["date_order", ">=", this.state.startDate2],
            ["date_order", "<=", this.state.endDate2]
          );
        }
      ...
      if (this.props.title === "pie1") {
        namaHeader = "Status Penawaran";
      } else {
        namaHeader = "Status Faktur";
      }
...
PenjualanChartRenderer.template = "owl.PenjualanChartRenderer";
