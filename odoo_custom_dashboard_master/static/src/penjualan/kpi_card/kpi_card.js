...
export class PenjualanKpiCard extends Component {
  setup() {
  ...
  async updateKpiData() {
    ...
    try {
      ...
      if (this.state.startDate) {
        domain.push(["date_order", ">=", this.state.startDate]);
        domain1.push(["date_order", ">=", this.state.startDate]);
        domain2.push(["date_order", ">=", this.state.startDate]);
        domain3.push(["date_order", ">=", this.state.startDate]);
      }
      if (this.state.endDate) {
        domain.push(["date_order", "<=", this.state.endDate]);
        domain1.push(["date_order", "<=", this.state.endDate]);
        domain2.push(["date_order", "<=", this.state.endDate]);
        domain3.push(["date_order", "<=", this.state.endDate]);
      }

      domain.push(["invoice_status", "=", "upselling"]);
      domain2.push(["invoice_status", "=", "to invoice"]);
      domain1.push(["invoice_status", "=", "invoiced"]);
      domain3.push(["state", "=", "draft"]);

      let orderData = await this.orm.call("sale.order", "search_read", [
        domain1,
        ["id", "partner_id"],
      ]);

      let fakturOrder = await this.orm.call("sale.order", "search_read", [
        domain2,
        ["id", "partner_id", "invoice_status"],
      ]);

      let upsellData = await this.orm.call("sale.order", "search_read", [
        domain,
        ["id", "name"],
      ]);

      let penawaranData = await this.orm.call("sale.order", "search_read", [
        domain3,
        ["id", "name", "state"],
      ]);

      console.log("Penawaran Data :", penawaranData);

      let faktur = fakturOrder.length;
      let penawaran = penawaranData.length;
      let orderTotal = orderData.length;
      let upsell = upsellData.length;
  ...
  async handleKpiCardClick(evt) {
    ...
    if (cardName === "Penawaran") {
      let { domain } = cardData;
      const actionId = "sale.action_quotations_with_onboarding";
      this.actionService
        .loadAction(actionId)
        .then((action) => {
          const newAction = {
...
PenjualanKpiCard.template = "owl.PenjualanKpiCard";
