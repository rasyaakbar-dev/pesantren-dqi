...
export class PerekrutanList extends Component {
  setup() {
  ...
  async fetchAllProducts() {
    try {
      ...
      if (this.state.startDate && this.state.endDate) {
        domainPelamar.push(["create_date", ">=", this.state.startDate]);
        domainPelamar.push(["create_date", "<=", this.state.endDate]);
      }
      ...
      console.log("Fetched Karyawan Data:", dataPelamar);
      this.state.pelamarr = this.getPelamarData(dataPelamar);
      await this.render();
    } catch (error) {
...
PerekrutanList.template = "owl.PerekrutanList";
