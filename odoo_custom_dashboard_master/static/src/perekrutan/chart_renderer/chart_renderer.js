...
export class PerekrutanChartRenderer extends Component {
  setup() {
  ...
  async fetchAndProcessData(startDate, endDate) {
    ...
    try {
      ...
      // ]);
      pie1 = await this.orm.call("hr.applicant", "search_read", [
        domain1,
        ["id", "partner_name", "stage_id"],
      ]);

      pie2 = await this.orm.call("hr.applicant", "search_read", [
        domain2,
  ...
  async processData(pie1, pie2, pie3) {
    const aggregateDataByStatus = (data) => {
      const stateCounts = {};
      data.forEach((record) => {
        const state = record.application_status;
    ...
    if (this.props.title === "pie1") {
      ...
      const stateCounts = aggregateDataByStatus(pie2);
      ...
      this.state.labels = Object.keys(stateCounts).map((state) => {
        if (state === "draft") return "Draf";
        if (state === "posted") return "Terekam";
  ...
  handleChartClick(evt) {
    ...
    if (activePoints.length > 0) {
      ...
      if (this.props.title === "pie1") {
        let tahapanName = originalLabel;
        domainAction.push(
          ["id", "in", associatedIds],
          ["stage_id", "=", tahapanName]
        );
      } else if (this.props.title === "pie2") {
        let namaLamaran = originalLabel;
        let convertLamaranName;

        if (namaLamaran === "Berjalan") {
          convertLamaranName = "ongoing";
        } else if (namaLamaran === "Diterima") {
          convertLamaranName = "hired";
        } else if (namaLamaran === "Diarsipkan") {
          convertLamaranName = "archived";
        }
        if (convertLamaranName) {
          domainAction.push(
            ["id", "in", associatedIds],
            ["application_status", "=", convertLamaranName],
...
PerekrutanChartRenderer.template = "owl.PerekrutanChartRenderer";
