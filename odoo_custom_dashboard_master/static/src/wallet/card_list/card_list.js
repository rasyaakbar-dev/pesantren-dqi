...
export class WalletCardList extends Component {
  setup() {
  ...
  async fetchAllProducts() {
    try {
      ...
      const productData = await this.orm.call("cdn.uang_saku", "search_read", [
        dateDomain,
        [
          "id",
          "siswa_id",
  ...
  getTopProducts(productData) {
    ...
    const uniqueProducts = sortedData.reduce((acc, data) => {
      if (!acc[data.id]) {
        acc[data.id] = {
          siswa: data.siswa_id ? data.siswa_id[1] || "N/A" : "N/A",
          kelas: data.kelas_id
            ? data.kelas_id[1] || "Santri belum Memiliki Kelas" // Fixed: menggunakan kelas_id bukan siswa_id
            : "Santri belum Memiliki Kelas",
          va_saku: data.va_saku,
          jns_transaksi: data.jns_transaksi,
          keterangan: data.keterangan || "Keterangan tidak diatur",
    ...
    return Object.values(uniqueProducts)
      ...
      .map((data, index) => ({
        ...
        create_date: this.formatDate(data.create_date),
      }));
  }
...
export class WalletCardList2 extends Component {
  setup() {
    this.orm = useService("orm");
    this.action = useService("action");
    const today = new Date();
  ...
  async fetchAllProducts() {
    try {
      const productData = await this.orm.call("cdn.siswa", "search_read", [
        [],
        ["id", "name", "nis", "ruang_kelas_id", "saldo_uang_saku"],
      ]);
  ...
  getTopProducts(productData) {
    const santriWithSaldo = productData.filter((santri) => {
      return santri.saldo_uang_saku && santri.saldo_uang_saku > 0;
    });

    const sortedBySaldo = santriWithSaldo.sort((a, b) => {
      return (b.saldo_uang_saku || 0) - (a.saldo_uang_saku || 0);
    });

    return sortedBySaldo.slice(0, 10).map((santri, index) => ({
      number: index + 1,
      id: santri.id,
      name: santri.name || "Nama tidak tersedia",
      nis: santri.nis || "NIS tidak tersedia",
      kelas: santri.ruang_kelas_id
        ? Array.isArray(santri.ruang_kelas_id)
          ? santri.ruang_kelas_id[1]
          : santri.ruang_kelas_id
        : "Kelas tidak tersedia",
      saldo_uang_saku: santri.saldo_uang_saku || 0,

      formatted_saldo: this.formatCurrency(santri.saldo_uang_saku || 0),
    }));
  }
...
WalletCardList.template = "owl.WalletCardList";
WalletCardList2.template = "owl.WalletCardList2";
